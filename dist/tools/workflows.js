...
import { CreateWorkflowSchema, UpdateWorkflowSchema, ListWorkflowsSchema, RunWorkflowSchema, WorkflowTagsSchema, IdParamSchema } from '../schemas/index.js';
...
export const registerWorkflowTools = (server) => {
    // ============ List Workflows ============
...
Returns:
  ...
  Includes nextCursor for pagination if more results exist.`,
        ...
        const response = await get('/workflows', queryParams);
...
Example node types:
  ...
  - n8n-nodes-base.merge - Merge data`,
        ...
        const workflow = await post('/workflows', params);
        return {
...
Returns:
  The updated workflow object.`,
        ...
        const workflow = await put(`/workflows/${id}`, updateData);
        return {
...
Returns:
  The activated workflow.`,
        ...
        const workflow = await post(`/workflows/${params.id}/activate`);
        return {
...
Returns:
  The deactivated workflow.`,
        ...
        const workflow = await post(`/workflows/${params.id}/deactivate`);
        return {
    ...
    // ============ Run Workflow ============
    server.registerTool('n8n_run_workflow', {
...
Returns:
  ...
  - data: Output data from the workflow`,
        ...
        const execution = await post(`/workflows/${params.id}/run`, body);
    ...
    server.registerTool('n8n_update_workflow_tags', {
...
Returns:
  The updated workflow with new tags.`,
        ...
        const workflow = await put(`/workflows/${params.id}/tags`, params.tags);
        return {
